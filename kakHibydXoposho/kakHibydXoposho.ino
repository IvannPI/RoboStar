...
void SetServoDegree(byte pin , uint8_t byAngle)
{
  int servoAngle;
  ServoPin = pin;
  if (byAngle > 360) {
  ...
  if (byAngle == 90 || byAngle == 270) {
    servoAngle = ServoBaseDegree;
  } else if (byAngle >= 0 && byAngle <= 180) {
    servoAngle = ServoBaseDegree - 90 + byAngle;   // 180-degree-diff
  }
  if (ServoPin == 1)
    mServo1->writeServo(servoAngle);
  else if (ServoPin == 2)
    mServo2->writeServo(servoAngle);
  else if (ServoPin == 3)
    mServo3->writeServo(servoAngle);
  else if (ServoPin == 4)
    mServo4->writeServo(servoAngle);
  else if (ServoPin == 5)
    mServo5->writeServo(servoAngle);
  else if (ServoPin == 6)
    mServo6->writeServo(servoAngle);
}
